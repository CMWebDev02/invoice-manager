...
export const userHomeDir = file_system.userHomeDir;
...
export async function validateSubDir(dirPath: string, subDir: string): Promise<string> {
  try {
    const isDirPathValid = await validateDirectoryPath(dirPath);
    if (!isDirPathValid) throw new Error('Invalid Directory Path', { cause: '004' });
    const yearSubDirPath = joinPaths(dirPath, subDir);
...
export async function validateFileName(fileName: string, dirPath: string): Promise<string> {
  try {
    let currentFileName = fileName;
    let isNameTaken = false;

    do {
      const newFilePath = joinPaths(dirPath, currentFileName);
...
export async function transferFile(currentFileName: string, currentFilePath: string, newDirPath: string): Promise<string> {
  try {
    ...
    const fileName = await validateFileName(currentFileName, newDirPath);

    const finalFilePath = joinPaths(newDirPath, fileName);
...
export async function getCurrentInvoice(invoicesDestination: string): Promise<FileExport | null> {
  try {
    ...
    const invoicesDirectoryContent = await getFiles(invoicesDestination);
...
export async function getSubDirectories(directoriesDestination: string): Promise<DirectoryExport[][]> {
  try {
    ...
    for (const subDir of subDirectoriesArray) {
      const subDirPath = await validateSubDir(directoriesDestination, subDir);
      subDirPaths.push(subDirPath);
    }

    for (const subDirPath of subDirPaths) {
      // Gathers all directories of the current sub directory
      const allDirectories = await getDirectories(subDirPath);
      ...
      const SubFolderObjArray = allDirectories.map(({ name, parentPath }) => {
        const dirPath = joinPaths(parentPath, name);
        return { name, dirPath };
      });
